...
struct Tile { // (i.e. movable piece)
    ...
    int weld = WALL_NONE; // only WALL_LEFT and WALL_UP are allowed
    IVec pos;
...
struct Cell {
    FloorType floor = FLOOR_VOID;
    Power floor_power;
    int weld = WALL_NONE; // only WALL_LEFT and WALL_UP are allowed
    int barrier = WALL_NONE; // retractable wall; only WALL_LEFT and WALL_UP are allowed
    int barrier_active = WALL_NONE;
...
template <typename T>
void save_pod(const T *x, size_t count, std::vector<char> &out) {
    size_t i = out.size();
    const size_t bytes = count * sizeof(T);
    out.resize(i + bytes);
    memcpy(&out[i], x, bytes);
}
template <typename T>
void load_pod(T *x, size_t count, const char **in, const char *end) {
    const size_t bytes = count * sizeof(T);
    if (end - *in < bytes)
        std::abort();
    memcpy(x, *in, bytes);
...
struct World {
    ...
    void save(std::vector<char> &out) const {
        size_t version = 1;
        save_pod(&version, 1, out);
        save_pod(&size, 1, out);
        for (int y = 1; y + 1 < size.y; ++y)
            save_pod(&cells[y][1], size.x - 2, out);
        size_t t = tiles.size();
        save_pod(&t, 1, out);
        save_pod(&tiles[0], t, out);
    }
    void load(const std::vector<char> &in_vec) {
        const char *in = &in_vec[0];
        const char *end = in + in_vec.size();
        size_t version = 0;
        load_pod(&version, 1, &in, end);
        if (version != 1)
            std::abort();
        load_pod(&size, 1, &in, end);
        ...
        for (int y = 1; y + 1 < size.y; ++y)
            load_pod(&cells[y][1], size.x - 2, &in, end);
        size_t t;
        load_pod(&t, 1, &in, end);
        if (t > 1000000)
            std::abort();
        tiles.resize(t);
        load_pod(&tiles[0], t, &in, end);
...
void draw_floor(const std::vector<std::vector<Cell>> &cells, IVec idx, Vector2 pos) {
    ...
    if (cell.floor == FLOOR_VOID) {
        ...
        int wall_wires = cell.floor_power.wires & WIRE_ALL_DIRECTIONS;
        if (wall_wires)
            draw_power({.wires = wall_wires, .power = cell.floor_power.power & wall_wires}, pos);
    }
